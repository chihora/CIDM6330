...
def request_with_backoff(
    ...
    for attempt in range(retries + 1):
        try:
            ...
            sleep_seconds = min(backoff_base_seconds * (2 ** attempt), 20)
            jitter = random.uniform(0.0, 0.10 * sleep_seconds)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Kata 3: FRED API consumption with pagination, retries, and output file"
    )
    parser.add_argument("--series-id", default="GDP", help="FRED series id (default: GDP)")
