...
def get_station(conn: sqlite3.Connection, station_id: str) -> Optional[Station]:
    """
...
def list_stations(conn: sqlite3.Connection) -> List[Station]:
    ...
    List all stations.
    """
    cur = conn.execute("SELECT station_id, name, state FROM stations ORDER BY station_id")
    return [Station(r["station_id"], r["name"], r["state"]) for r in cur.fetchall()]


def update_station_name(conn: sqlite3.Connection, station_id: str, new_name: str) -> int:
    """
...
def insert_observation(conn: sqlite3.Connection, obs: Observation) -> None:
    ...
    """
    conn.execute(
        "INSERT INTO observations (station_id, obs_date, temperature_c) VALUES (?, ?, ?)",
...
def list_observations_for_station(
    ...
    """
    List observations for a station between start_date and end_date (inclusive boundaries via BETWEEN).
    """
    cur = conn.execute(
        """
    ...
    return [Observation(r["station_id"], r["obs_date"], r["temperature_c"]) for r in cur.fetchall()]


def update_observation_temp(conn: sqlite3.Connection, station_id: str, obs_date: str, new_temp: float) -> int:
    """
...
def create_indexes(conn: sqlite3.Connection) -> None:
    ...
    """
    conn.execute("CREATE INDEX IF NOT EXISTS idx_obs_station_date ON observations(station_id, obs_date)")
    conn.commit()
